...
class DailyInvoice implements DataInterface
{
    public  function getData($params, $connection)
    {
        ...
        $invoice_header = collect(DB::connection($connection)->select("SELECT i.acKey, convert(varchar(20),i.adDate,104) as adDate, s.acName, 
        s.acAddress, s.acCity, s.acPost,s.acPhone, s.acRegNo,s.acCode, s.anSubjectTypeId, pc.acPayCondition, pt.acName as acPayType
        , u.acName + ' ' +isnull(u.acSurname,'') as acUser	, i.anReservationId, i.anValue, i.anVatValue, i.anTotalValue, 
        i.anValueRSD, i.anVatValueRSD, i.anTotalValueRSD,i.anFxRate,i.acComment,cast(v.anVat as int) as anVat
        ,i.anTypeId, i.anBI
        from _Invoices i
        ...
        $car = collect(DB::connection($connection)->select("SELECT c.acRegNo, cb.acBrand + ' ' + cm.acModel + ' ' + cv.acVersion  as fullname,
        cb.acBrand   as brand
		,cm.acModel as model
