...
class InternalReservationDetail implements DataInterface
{
    public  function getData($params, $connection)
    {
        ...
        $title = "Pregled internih rezervacija - hronoloski";
        ...
        $contracts = collect(DB::connection($connection)->select("SELECT r.acKey as Dokument
        ,sd.acSubDocType as Tip
        ,c.acRegNo as Vozilo
        ,convert(varchar(20), r.adDateFrom,104)+' '+convert(varchar(8),convert(time,coalesce( r.adDateFrom,108))) as [Od]
        ,convert(varchar(20), r.adDateTo,104)+' '+convert(varchar(8),convert(time,coalesce( r.adDateTo,108))) as [Do]
        ,dd.acName as Vozac
        ...
        inner join _Drivers dd on r.anDriverId = dd.anId
