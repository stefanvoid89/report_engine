...
class InvoiceRecDetail implements DataInterface
{
    public  function getData($params, $connection)
    {
        ...
        , sum(case when irt.acType = 'lizing' then cast(ii.anTotalValue * 0.83333333 as decimal(19,2)) else ii.anValue end)  as Vrednost
		, sum(case when irt.acType = 'lizing' then cast(ii.anTotalValue * 0.16666666 as decimal(19,2)) else ii.anVatValue end) as PDV
		, sum(ii.anTotalValue) as Total
        , sum(case when irt.acType = 'lizing' then cast(ii.anTotalValueRSD * 0.83333333 as decimal(19,2)) else ii.anValueRSD end) as Vrednost_RSD
		,sum(case when irt.acType = 'lizing' then cast(ii.anTotalValueRSD * 0.16666666 as decimal(19,2)) else ii.anVatValueRSD end) as PDV_RSD 
        ...
        $parameters =   $date_from_param  .  $date_to_param . $brand_param . $model_param . $car_type_param . $car_category_param . $car_param . $invoice_rec_doc_param . $invoice_param . $invoice_type_param . $subject_type_param . $subject_param;
