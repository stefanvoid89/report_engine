...
class DailyInvoice implements DataInterface
{
    public  function getData($params, $connection)
    {
        ...
        $invoice_header = collect(DB::select("SELECT i.acKey, convert(varchar(20),i.adDate,104) as adDate, s.acName, s.acAddress, s.acCity, s.acPost,s.acPhone, s.acRegNo,s.acCode, pc.acPayCondition, '' as acPayType
...
where i.anId =:id", ['id' => $id]))->first();
        ...
        $car = collect(DB::select("SELECT c.acCarNameShort as acName, c.acRegNo, c.acChasis	 from _v_CarExtended c inner join _Reservations r on r.anCarId = c.anId
where r.anId = :reservation_id", ['reservation_id' => $reservation_id]))->first();
        ...
        if ($_currency == 'eur') {
            ...
            $positions =  collect(DB::select("SELECT ROW_NUMBER() over (order by ii.anId) as anNo, si.acIdent, ii.acName, ii.acUm, ii.anQty, ii.anPrice,ii.anRebate,ii.anValue  from _InvoiceItems ii
